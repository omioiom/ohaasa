...
# ==========================================
# [공통] 매핑 테이블 및 디자인 설정 (원래 디자인 100% 유지)
# ==========================================
SIGN_MAP = {
...
# ==========================================
# [기능 2] 이미지 생성 헬퍼 (사용자 원래 디자인 로직 100% 유지)
# ==========================================
...
def post_to_instagram_reels(video_path, caption):
    ...
    #         st_code = status.get("status_code", "")
    #         print(f"릴스 상태: {st_code}")
    #         if st_code == "FINISHED":
    #             time.sleep(10)
...
def main():
    ...
    try:
        ...
        #
        #     post_to_instagram_reels(video_final, f"🔮 {date_display} 오하아사 별자리 운세")
        # except Exception as ve: print(f"영상 오류: {ve}")

        # 4. 피드 포스팅
        public_urls = []
        for p in image_paths:
            u = upload_to_litterbox(p)
            if u: public_urls.append(u)
        
        if len(public_urls) == len(image_paths):
            post_to_instagram(public_urls, f"🔮 {date_display} 오하아사 오늘의 운세")
        else:
            print("일부 이미지 업로드 실패로 피드 포스팅 취소")
